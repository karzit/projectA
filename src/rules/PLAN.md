...
### 2026-07-10 사거리 차폐 모델 철회 — 고정 사거리 + "적 없으면 사거리 +1"

- 사용자 판단: "빈 칸은 거리 0" 차폐 규칙이 비직관적 — 2026-07-02 도입한
  차폐 모델을 되돌림. `attackableTargets`(`queries.ts`)가 다시 자기 레인의
  **상대 전열 유닛**을 기본 대상으로 반환 (후열 공격자도 아군 전열에 안 막힘).
- 같은 날 후속 규칙 추가: **기본 사거리 안에 공격 가능한 적이 하나도 없으면
  사거리 +1** — 레인 뒤의 상대 **후열**까지 대상이 된다 (레인 전열에 적이
  하나라도 있으면 후열은 사거리 밖). 미공개/트랩 유닛은 `isRevealed`상
  "공격 가능한 적"이 아니므로 확장을 막지 않는다.
- `attackableTargets`가 단일 소스라 gameCore 검증·SimAI 모두 자동 반영.
  테스트(`rules-loop.test.ts` 사거리 블록 5종) 갱신, 문서 3종(`RULES.md`,
  `src/rules/README.md`, 루트 `CLAUDE.md`) 갱신.
...
- [ ] **D-1.** 재미/방향성 점검 — AI 자동 대전(`tests/ai-balance.test.ts`, SimAI/MctsAI
      양쪽에 실제 룰 엔진을 붙여 대량 시뮬레이션) 기반으로 23회차(2026-07-03~10)에
      ...
      - **버그 수정으로 해소된 것**: 협공 reaction window 미해석, 오프닝 휴리스틱의
        ...
        AI가 안 쓰던 평가 함수 자기모순(reserveExempt 도입), heroic의 마왕성 입성
        자멸 러시(chainGate + Hero XP/지략봉쇄 평가 신설로 해소), cult의
        `reserveCells`가 사교도 재플레이까지 막아 소굴 소실 시 25턴 pass 스톨에
        빠지던 버그(`cultist`를 `reserveExempt`에 추가) 등 — 전부 코드 수정으로
        해결, 안전판(무한 교착) 0/540 게임까지 확인. 23회차 종료 시 3덱 승률
        heroic/journey/cult 47.9/49.6/52.5%로 자연 수렴(너프가 아닌 버그 수정
        결과 — 22회차 요약에서 남겼던 "heroic 완주 후 cult전 패배" 이슈 해결).
