...
// 마법사[1/4, 결속, 배경:용사]. 용사가 두 번 레벨업할 때마다 +1/+1.
...
class MageCard extends UnitCard {
  readonly meta: CardMeta = {
    ...
    desc: '결속. 용사가 두 번 레벨업할 때마다 +1/+1. 공격 대신 무작위 적의 힘을 지혜 25%만큼 턴 종료까지 감소.',
  };
  ...
  override subscribe(ctx: GameContext): void {
    ...
    ctx.events.on({
      ...
      fire: (ev) => {
        if (ev.kind !== 'heroLevelUp') return;
        if (!ctx.board.getUnit(unitId)) return;
        // 짝수 레벨에서만 발동 → 두 번의 레벨업마다 한 번 +1/+1.
        if (ev.level % 2 !== 0) return;
  ...
  override onAbility(ctx: GameContext): void {
    if (!ctx.unitId) return;
    const [enemyId] = ctx.board.pickRandom('oppField', ctx.controller, 1);
    if (!enemyId) return;
