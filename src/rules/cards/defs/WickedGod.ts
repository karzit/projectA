...
// 최후: 이번 게임동안 내 모든 사특한 신이 +1/+1을 얻습니다.
class WickedGodCard extends UnitCard {
  readonly meta: CardMeta = {
    ...
    wisdom: 6,
    keywords: ['사특한 신'],
    desc: '등장: 내 사교도 사망 시 사특한 신 소환. 최후: 모든 아군 사특한 신 +1/+1.',
  };
  ...
  override onDeath(ctx: GameContext): void {
    ...
    for (const u of ctx.board.unitsOn(controller)) {
      if (u.cardId === 'wicked-god') {
        ctx.board.modifyStat(u.instanceId, 'power', 1);
        ctx.board.modifyStat(u.instanceId, 'wisdom', 1);
      }
    }
  }
}
